...
use mezame::config::AgentConfig;
use serde_json::json;
use tokio::time::timeout;

fn cat_agent() -> AgentConfig {
    AgentConfig {
        name: "cat".into(),
...
async fn spawn_agent_returns_handle_and_updates_channel() {
    let result = timeout(Duration::from_secs(5), spawn_agent(&cat_agent())).await;
...
async fn spawn_agent_routes_responses_back_through_request() {
    ...
    let (agent, mut updates_rx) = spawn_agent(&cat_agent()).await.expect("spawn_agent");
...
async fn spawn_agent_errors_when_command_does_not_exist() {
    let mut agent_cfg = cat_agent();
    agent_cfg.command = "this-binary-definitely-does-not-exist-xyz123".into();

    let res = timeout(Duration::from_secs(5), spawn_agent(&agent_cfg))
        .await
